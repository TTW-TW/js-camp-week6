...
async function getProducts() {
    ...
    // 3. 回傳 data.products
    const apiUrl = `/api/livejs/v1/customer/${API_PATH}/products`;
...
async function getCart() {
    // 請實作此函式
    const apiUrl = `/api/livejs/v1/customer/${API_PATH}/carts`;
    const fullUrl = BASE_URL + apiUrl;
...
async function removeCartItem(cartId) {
    ...
    const apiUrl = `/api/livejs/v1/customer/${API_PATH}/carts/${cartId}`;
    const fullUrl = BASE_URL + apiUrl;
...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   ...
   1xx：收到資訊正在處理中
   2xx：請求成功，例如 200 OK 
   3xx：重新導向，需要進行其他操作才能完成
   4xx：客戶端有問題，請求語法錯誤、被禁止或因故無法完成，例如 403 Forbidden、404 Not Found
   5xx：伺服器端有問題，處理請求發生錯誤

2. GET、POST、PATCH、PUT、DELETE 的差異
   答：
   上述都是 API 的請求 Method
   GET：取得資料，不帶參數
   POST：新增資料，需攜帶參數 (若 id 相同會整合)
   PATCH：編輯(修改資料)，參數只需帶要修改的欄位
   PUT：編輯(修改資料)，參數要帶完整的欄位(即使該欄位沒有要修改)
   DELETE：刪除 (整個購物車，或刪除指定物件)

3. 什麼是 RESTful API？
   答：
   基於 Rest 架構風格的網路應用程式介面設計標準，
   透過 http 方法操作資源 (例如 GET、POST、PATCH、PUT、DELETE)，
   跟一般 API 相比，URL 的命名方法較為統一且直觀，
   讓不同系統能安全、有效率地進行資料交換
